...
// Disconnect old native Windows library to prevent external linking conflicts
#pragma comment(linker, "/NODEFAULTLIB:scrnsave.lib")

// --- MANDATORY STUBS FOR COMPATIBILITY ---
extern "C" {
...
int g_BrightPercent = 100;    // Default control for bright glyphs percentage
...
void ResetChain(Chain& c) {
    ...
    // Begins strictly from the 5th to last position (avoiding the 4 fading ones)
    c.brightRow = (float)((int)c.headRow - c.maxLength + 4);
    // Multiplier safely bounded between a minimum of 2.5x and a maximum of 4x (15 * 0.1 = 1.5)
    ...
    case WM_CREATE: {
        ...
        for (int i = 0; i < dynamicDensity; ++i) {
            ...
            if (chains[i].maxLength > 0 && chains[i].brightActive) {
                // Initial random positioning respecting the restriction of the last 4 characters
        ...
        for (const auto& ch : chains) {
            ...
            for (int r = 0; r < g_MaxRows; ++r) {
                if (r >= currentTail && r <= currentHead) {
                    ...
                    // Strict restriction: bright flashes are only rendered from the 5th to last onward (r >= currentTail + 4)
            ...
            if (ch.brightActive) {
                ...
                if (ch.brightRow > ch.headRow + 2) {
                    if (rand() % 100 < g_BrightPercent) {
                        // When recycling the flash, skip the first 4 transparent characters from the tail start
...
LRESULT CALLBACK ConfigWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Font Size (Pixels):", WS_CHILD | WS_VISIBLE, 25, 10, 280, 20, hWnd, NULL, NULL, NULL);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Chain Density (Max Extended x2):", WS_CHILD | WS_VISIBLE, 25, 65, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Fall Speed (Limits 1/5 of Total):", WS_CHILD | WS_VISIBLE, 25, 120, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Graphics Quality (Low - Medium - High):", WS_CHILD | WS_VISIBLE, 25, 175, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Change Speed (Glyph Mutation):", WS_CHILD | WS_VISIBLE, 25, 230, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Mutable Glyphs Amount (%):", WS_CHILD | WS_VISIBLE, 25, 285, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Minimum Chain Length:", WS_CHILD | WS_VISIBLE, 25, 340, 280, 20, hWnd, NULL, NULL, NULL);
        ...
        hCtrl = CreateWindowW(L"STATIC", L"Maximum Chain Length:", WS_CHILD | WS_VISIBLE, 25, 395, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        hCtrl = CreateWindowW(L"BUTTON", L"Change Base Color", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 25, 455, 280, 30, hWnd, (HMENU)201, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);

        hCtrl = CreateWindowW(L"STATIC", L"Bright Glyphs Amount (%):", WS_CHILD | WS_VISIBLE, 25, 495, 280, 20, hWnd, NULL, NULL, NULL);
        SendMessageW(hCtrl, WM_SETFONT, (WPARAM)hDialogFont, TRUE);
        ...
        // Row 1 - Column 1
        hMulticolorCheck = CreateWindowW(L"BUTTON", L"Per-Glyph Multicolor", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 25, 555, 140, 20, hWnd, (HMENU)202, NULL, NULL);
        ...
        // Row 1 - Column 2
        hMulticolorLineCheck = CreateWindowW(L"BUTTON", L"Per-Line Multicolor", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 170, 555, 140, 20, hWnd, (HMENU)204, NULL, NULL);
        ...
        // Row 2 - Column 1
        hRandomSpeedCheck = CreateWindowW(L"BUTTON", L"Random Speed", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 25, 580, 140, 20, hWnd, (HMENU)203, NULL, NULL);
        ...
        // Row 2 - Column 2
        hReduceGapsCheck = CreateWindowW(L"BUTTON", L"Reduce Gaps", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 170, 580, 140, 20, hWnd, NULL, NULL, NULL);
        ...
        hMinRandomLabel = CreateWindowW(L"STATIC", L"Minimum Random Speed:", WS_CHILD, 25, 610, 280, 20, hWnd, NULL, NULL, NULL);
        ...
        hMaxRandomLabel = CreateWindowW(L"STATIC", L"Maximum Random Speed:", WS_CHILD, 25, 665, 280, 20, hWnd, NULL, NULL, NULL);
        ...
        hCharLabel = CreateWindowW(L"STATIC", L"Matrix Characters (Comma separated):", WS_CHILD | WS_VISIBLE, 25, 610, 280, 20, hWnd, NULL, NULL, NULL);
        ...
        hOkButton = CreateWindowW(L"BUTTON", L"Save", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 45, 740, 100, 32, hWnd, (HMENU)IDOK, NULL, NULL);
        SendMessageW(hOkButton, WM_SETFONT, (WPARAM)hDialogFont, TRUE);

        hCancelButton = CreateWindowW(L"BUTTON", L"Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 175, 740, 100, 32, hWnd, (HMENU)IDCANCEL, NULL, NULL);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show) {
    ...
    if (!arguments.empty() && (arguments.find("/c") == 0 || arguments.find("/C") == 0 || hParentWnd == NULL && g_IsPreview == false && arguments.find("/s") == std::string::npos && arguments.find("/S") == std::string::npos)) {
        ...
        HWND hDlg = CreateWindowExW(WS_EX_DLGMODALFRAME, L"MatrixConfigWindow", L"Matrixmio Properties",
